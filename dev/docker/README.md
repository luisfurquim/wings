...
> **⚠️ Experimental (v0.16.7-alpha).** The `cmd/build dev` loop is tested and
...
| -------------------- | ---------------------- | -------------------------------------------------------------- |
| `WINGS_VERSION`      | `v0.16.7-alpha`        | wings version for the dev tool. The loop bumps your `go.mod` up to it (see below). |
